...
public class RRDiscordBridge {
     ...
     * <p>
     * It's literally just a color used for some Discord messages
     */
     ...
     * <p>
     * Serves as a way to get server uptime.
     */
     ...
     * <p>
     * Because some fields need access to the {@link #instance static instance of RRDiscordBridge}, you must call {@link #initialize} after setting the static instance to fully initialize the plugin.
     *
    ...
    public RRDiscordBridge initialize() {
        try {
            ...
            if (!this.settings.consoleChannelId.isEmpty()) {
                ...
                this.logHandler = new ChannelLoggingHandler(() -> DiscordBot.jda.getTextChannelById(this.settings.consoleChannelId), config -> {
        ...
        RRDiscordBridge.instance.getLogger().info(String.format("RRDiscordBridge v%s has started.", RRDiscordBridge.instance.getVersion()));
