    ...
    public boolean invoke(ICommandCaller caller, String[] params) {
        ...
        switch (commandName.toLowerCase()) {
            ...
            case "info":
                ...
                caller.respond(String.format("%s%s§b - §r%s", RRDiscordBridge.instance.getChatExtensions().isEnabled(iExt) ? "§a" : "§c", iExt.getName(), iExt.getDescription()));
                break;
            case "list":
                caller.respond(String.format("§bChat Extensions (%s enabled, %s total)", RRDiscordBridge.instance.getChatExtensions().enabledExtensions.size(), RRDiscordBridge.instance.getChatExtensions().extensions.size()));
                // print enabled first
                for (IChatExtension ext : RRDiscordBridge.instance.getChatExtensions().enabledExtensions) {
                    caller.respond("§a" + ext.getName());
                }

                for (IChatExtension ext : RRDiscordBridge.instance.getChatExtensions().extensions) {
                    if (!RRDiscordBridge.instance.getChatExtensions().enabledExtensions.contains(ext))
                        caller.respond("§c" + ext.getName());
                }
                break;
            case "disable":
                ...
                RRDiscordBridge.instance.getChatExtensions().disable(RRDiscordBridge.instance.getChatExtensions().getExtension(dExtName));
            ...
            case "enable":
                ...
                RRDiscordBridge.instance.getChatExtensions().enable(RRDiscordBridge.instance.getChatExtensions().getExtension(extName));
