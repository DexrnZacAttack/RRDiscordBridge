...
public class SupportedFeatures {
    ...
    public SupportedFeatures setCanGetServerName(boolean v) {
        if (!v)
            RRDiscordBridge.instance.getLogger().warning("server.properties' server-name is not supported in this environment. There will be no server name when /about is used.");
        hasServerName = v;
        return this;
    }
    ...
    public SupportedFeatures setCanGetServerMotd(boolean v) {
        if (!v)
            RRDiscordBridge.instance.getLogger().warning("MOTD is not supported in this environment. There will be no MOTD when /about is used.");
        hasServerMotd = v;
        return this;
    }
