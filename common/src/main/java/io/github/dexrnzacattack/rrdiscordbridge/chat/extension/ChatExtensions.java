...
import io.github.dexrnzacattack.rrdiscordbridge.chat.extension.extensions.OpChatChatExtension;
import io.github.dexrnzacattack.rrdiscordbridge.chat.extension.extensions.WaypointChatExtension;
...
public class ChatExtensions {
    ...
    private void register(Class<? extends IChatExtension> ext, String name) {
        try {
            ...
            extensions.add(inst);
            if (RRDiscordBridge.instance.getSettings().enabledChatExtensions.contains(name)) {
                enabledExtensions.add(inst);
                inst.onEnable();
            }
        } catch (Exception e) {
            RRDiscordBridge.instance.getLogger().log(Level.SEVERE, String.format("Failed to register chat extension %s: %s", name, e.toString()), e);
        } finally {
            RRDiscordBridge.instance.getLogger().log(Level.INFO, String.format("Registered chat extension %s", name));
        }
    }
    ...
    void unregister(IChatExtension ext) {
        ...
        RRDiscordBridge.instance.getLogger().log(Level.INFO, String.format("Unregistered chat extension %s", ext.getName()));
    }
    ...
    public void disable(IChatExtension ext) {
        ...
        RRDiscordBridge.instance.getLogger().log(Level.INFO, String.format("Disabled chat extension %s", ext.getName()));
    }
    ...
    public void enable(IChatExtension ext) {
        ...
        RRDiscordBridge.instance.getLogger().log(Level.INFO, String.format("Enabled chat extension %s", ext.getName()));
    }
}
