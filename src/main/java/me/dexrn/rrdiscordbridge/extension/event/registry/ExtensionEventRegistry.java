...
public class ExtensionEventRegistry {
    ...
    private final Map<
                    ExtensionEventType<? extends ExtensionEvent>,
                    List<EventHandlerHolder<? extends ExtensionEvent>>>
            events = new HashMap<>();
    ...
    /** Registers an event */
    public <T extends ExtensionEvent> EventHandlerHolder<? extends ExtensionEvent> register(
            AbstractBridgeExtension owner,
            ExtensionEventType<T> type,
            IExtensionEventHandler<T> handler) {
        EventHandlerHolder<T> t = new EventHandlerHolder<>(owner, type, handler);
        events.computeIfAbsent(type, e -> new ArrayList<>()).add(t);

        return t;
    }

    /** Unregisters an event handler */
    public <T extends ExtensionEvent> void unregister(
            EventHandlerHolder<? extends ExtensionEvent> r) {
    ...
    public <T extends ExtensionEvent> void invoke(ExtensionEventType<T> type, T event) {
        for (EventHandlerHolder<? extends ExtensionEvent> h :
                events.getOrDefault(type, Collections.emptyList())) {
