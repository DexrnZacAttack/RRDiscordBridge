...
import static io.github.dexrnzacattack.rrdiscordbridge.RRDiscordBridge.configDir;
...
public class JarExtensionLoader implements IExtensionLoader {
    ...
    public void registerExtensions(BridgeExtensionManager manager) {
        ...
        URL[] jars = Arrays.stream(Objects.requireNonNull(e.listFiles()))
                .filter(j -> j.getName().endsWith(".jar"))
                .map(j -> {
                    try {
                        return j.toURI().toURL();
                    } catch (MalformedURLException ex) {
                        logger.error(String.format("Failed to load extension from %s: ", j.getName()), ex);
                        return null;
                    }
                })
